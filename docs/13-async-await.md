...
- **await**: solo dentro de una función async. Pausa la función hasta que la promesa se resuelva; el valor de la expresión es el valor resuelto. Si la promesa se rechaza, se lanza la excepción y se puede capturar con try/catch.
...
```
- **catch en la promesa devuelta**: quien llama a la función async puede usar .catch() en la promesa.
```js
cargar().catch(e => mostrarError(e));
```
- No mezclar mal: si dentro del async no hay try/catch y la promesa se rechaza, la promesa devuelta por la función async se rechazará.
